...
class AdvancedFileView extends View
  ...
  initialize: (serializeState) ->
    atom.commands.add "atom-workspace",
      "atom-include-path:toggle": => @toggle()
    atom.commands.add @element,
      "core:confirm": => @confirm()
      "core:cancel": => @detach()
      "atom-include-path:autocomplete": => @autocomplete()
      "atom-include-path:undo": => @undo()
      "atom-include-path:move-cursor-down": => @moveCursorDown()
      "atom-include-path:move-cursor-up": => @moveCursorUp()
  ...
  suggestPath: ->
    suggestedPath = ''
    switch atom.config.get("atom-include-path.defaultInputValue")
